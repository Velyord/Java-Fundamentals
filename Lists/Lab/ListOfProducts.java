...
public class ListOfProducts {
    ...
    public static void main(String[] args) {
        int countOfProducts = setValue(0, Integer.MAX_VALUE);
        List<String> productList = new ArrayList<>();

        addProductsToList(productList, countOfProducts);
        Collections.sort(productList);
        printNumberedList(productList);
    }

    private static void printNumberedList(List<String> productList) {
        for (int i = 0; i < productList.size(); i++) {
            out.printf("%d.%s%n", i + 1, productList.get(i));
        }
    }

    private static void addProductsToList(List<String> productList, int countOfProducts) {
