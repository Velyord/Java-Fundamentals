...
public class Train {
    ...
    public static void main(String[] args) {
        List<Integer> wagonPassengersList = Arrays
                .stream(setValue()
                        .split(" "))
                .map(Integer::parseInt)
                .collect(Collectors.toList());
        int wagonMaxCapacity = setValue(0, Integer.MAX_VALUE);
        String input = setValue();

        while(!input.equals("end")) {
            if (input.startsWith("Add ")) {
                int passengers = Integer.parseInt(input.substring(4));
                wagonPassengersList.add(passengers);
            } else {
                int passengers = Integer.parseInt(input);
                findEnoughBigWagon(passengers, wagonPassengersList, wagonMaxCapacity);
            }

            input = setValue();
        }

        wagonPassengersList.forEach(wagon -> out.print(wagon + " "));
    }

    private static void findEnoughBigWagon(
            int passengers,
            List<Integer> wagonPassengersList,
            int wagonMaxCapacity
    ) {
        for (Integer wagon : wagonPassengersList) {
            if (wagon + passengers <= wagonMaxCapacity) {
                wagonPassengersList.set(wagonPassengersList.indexOf(wagon), wagon + passengers);
                break;
            }
        }
    }
