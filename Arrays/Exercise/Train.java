...
public class Train {
    ...
    public static void main(String[] args) {
        int countOfWagons = setValue(0, Integer.MAX_VALUE);
        int[] peopleOnTrain = populateWithPeopleFrom(countOfWagons);

        displayPeopleInWagons(peopleOnTrain);
    }

    private static void displayPeopleInWagons(int[] peopleOnTrain) {
        int sumOfPeople = 0;

        for (int peopleOnWagon : peopleOnTrain) {
            sumOfPeople += peopleOnWagon;
            out.print(peopleOnWagon + " ");
        }

        out.println("\n" + sumOfPeople);
    }

    private static int[] populateWithPeopleFrom(int countOfWagons) {
        int[] peopleOnTrain = {};

        for (int i = 1; i <= countOfWagons; i++) {
            peopleOnTrain = addIntToArray(
                    setValue(Integer.MIN_VALUE, Integer.MAX_VALUE),
                    peopleOnTrain
            );
        }

        return peopleOnTrain;
    }
