...
public class BurgerBus {
    ...
    public static void main(String[] args) {
        int numberOfCitiesVisited = setValue(0, Integer.MAX_VALUE);
        int cityCount = 0;
        double totalProfit = 0;

        for (int i = 1; i <= numberOfCitiesVisited; i++) {
            cityCount++;

            String cityName = setValue();
            double earnedMoney = setValue(0.0, Double.MAX_VALUE);
            double expenses = setValue(0.0, Double.MAX_VALUE);
            double eventExpenses = 0;
            boolean isSpecialEvent = cityCount % 3 == 0;
            boolean isRaining = cityCount % 5 == 0;

            if (isSpecialEvent) {
                eventExpenses = expenses / 2;
            }
            ...
            double profit = earnedMoney - expenses - eventExpenses;
            totalProfit += profit;

            out.printf("In %s Burger Bus earned %.2f leva.\n", cityName, profit);
        }
        out.printf("Burger Bus total profit: %.2f leva.", totalProfit);
    }
