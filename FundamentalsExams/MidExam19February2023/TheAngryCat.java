...
public class TheAngryCat {
    ...
    public static void main(String[] args) {
        int[] priceRatings = Arrays
                .stream(setValue().split(", "))
                .mapToInt(Integer::parseInt)
                .toArray();

        int entryPoint = setValue(1, priceRatings.length - 2);
        String typeOfItems = setValue();
        int leftDamage = 0;
        int rightDamage = 0;
        ...
        displayDamage(leftDamage, rightDamage);
    }

    private static void displayDamage(int leftDamage, int rightDamage) {
        if (rightDamage > leftDamage) {
            out.printf("Right - %d", rightDamage);
        } else {
            out.printf("Left - %d", leftDamage);
        }
    }
    ...
    private static <T> boolean hasValidChars(T value) {
        ...
        String specialChars = "!#$%&'()*+/.:;<=>?@[]^_`{|}ABCDEFGHIJKLMNOPQRSTUVWXYZ"; // може да се променят забранените символи
    ...
    private static <T> boolean doesFollowTemplate(T value) {
        ...
        if (stringCount == 1) {
            ...
            requiredStrings = new String[]{"cheap", "expensive"}; // на втория низ. Празно ако няма такъв.
