...
public class Orders {
    ...
    public static void main(String[] args) {
        String order = setValue();
        int quantity = setValue(0, Integer.MAX_VALUE);

        printTotalPriceFor(order, quantity);
    }

    private static void printTotalPriceFor(String order, int quantity) {
        double totalPrice = getPriceFor(order) * quantity;

        out.printf("%.2f", totalPrice);
    }

    private static double getPriceFor(String order) {
        double price = 0.00;
    ...
    private static <T> boolean doesFollowTemplate(T value) {
        ...
        if (stringCount == 1) {
            requiredStrings = new String[]{"coffee", "water", "coke", "snacks"}; // на първия низ. Празно ако няма такъв.
