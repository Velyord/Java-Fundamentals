...
public class Calculations {
    ...
    public static void main(String[] args) {
        String operation = setValue();
        int firstNum = setValue(Integer.MIN_VALUE, Integer.MAX_VALUE);
        int secondNum = setValue(Integer.MIN_VALUE, Integer.MAX_VALUE);
        int result = calculate(firstNum, operation, secondNum);

        out.println(result);
    }

    private static int calculate(int firstNum, String operation, int secondNum) {
        int result = 0;
    ...
    private static <T> boolean doesFollowTemplate(T value) {
        ...
        if (stringCount == 1) {
            requiredStrings = new String[]{"add", "multiply", "subtract", "divide"}; // на първия низ. Празно ако няма такъв.
