...
public class GreaterOfTwoValues {
    ...
    private static void getBiggerOf(String type) {
        switch (type) {
            case "int":
                int firstInt = setValue(Integer.MIN_VALUE, Integer.MAX_VALUE);
                int secondInt = setValue(Integer.MIN_VALUE, Integer.MAX_VALUE);
                int biggerInt = getMax(firstInt, secondInt);

                out.println(biggerInt);
                break;
            case "char":
                char firstChar = setValue().charAt(0);
                char secondChar = setValue().charAt(0);
                char biggerChar = getMax(firstChar, secondChar);

                out.println(biggerChar);
                break;
            case "string":
                String firstString = setValue();
                String secondString = setValue();
                String biggerString = getMax(firstString, secondString);

                out.println(biggerString);
                break;
            default:
                out.println("Error! Invalid type. Restart the program!");
                break;
        }
    }
    ...
    private static <T> boolean doesFollowTemplate(T value) {
        ...
        if (stringCount == 1) {
            requiredStrings = new String[]{"int", "char", "string"}; // на първия низ. Празно ако няма такъв.
