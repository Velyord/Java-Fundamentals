...
Task:
    ...
    Whether the letter was in front of the number or after it,
    ...
    He complicated it a bit by doing the same
    but with multiple strings keeping track of only the total sum of all results.
    Once he started to solve this with more strings and bigger numbers,
    it became quite hard to do it only in his mind.
    So he kindly asks you to write a program that calculates the sum of all numbers
    ...
    So we divide the number 12 by position 1 (12/1 = 12).
    ...
    Then we have Uppercase G with position 7,
    subtracting it from the resulting number (323 – 7 = 316).
    Finally, we sum the 2 results and get 14 + 316=330.
Input:
...
Output:
    Print a single number at the console:
    the total sum of all processed numbers rounded to two digits after the decimal separator.
Constraints:
